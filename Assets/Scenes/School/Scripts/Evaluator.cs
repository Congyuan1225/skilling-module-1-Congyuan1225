...
public class Evaluator : MonoBehaviour {
    ...
    public void DisplayAge()
    {
        ...
        {
            int ageOfCube = child.gameObject.GetComponent<CA_Cube_Class_2>().GetAge();
        ...
        foreach(Transform child in caGrid.transform)
        {
            float greyVal = Remap((float) child.gameObject.GetComponent<CA_Cube_Class_2>().GetAge(), minVal, maxVal, 0.0f, 1.0f);
            Color greyColour = new Color(greyVal, greyVal, greyVal);
            child.GetComponent<CA_Cube_Class_2>().DisplayAge(greyColour);
        }
